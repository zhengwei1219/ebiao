...
**/
var FlowNodes = new Array();//保存节点对象，替换 FlowArea.FlowNodes.Nodes 属性对象 2012-10-25
...
function LoadFlowLittleIcon(oXml){
	...
	for (var i=0;i<oList.length;i++){
		var stepType = oList[i].childNodes[0].text	; // 当前步骤；历史步骤
...
function SetStepInfoToIcon(oXml){
	...
	for (var i=oList.length-1;i>=0;i--){
		...
		var subwfid = oList[i].childNodes[13].text;//子流程实例id 2012-1-6
...
function Flow_BuildResult(FlowArea,strResultId,strFromId, strEndId,strOuterHTML){
	...
	if (browser.indexOf("msie")>=0){
		...
		//id=8043 NodeType="Result" Name="连线节点" ToNode="4493" FromNode="3352"
	...
	activeNode.strokecolor = "#030"; //vml的属性
...
function Flow_BuildActiveNode(FlowArea,strNodeID, strNodeName, strNodeType, strOuterHTML){
	var browser = GetBrowserKey();
	if (browser.indexOf("msie")<0){//在safari上font节点不支持width属性，换成div
	...
	if (activeNode.getAttribute("NodeType") =="StartNode"){//显示打勾
