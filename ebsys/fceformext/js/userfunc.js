...
var fcuser={
	...
	groupParentId    : "",groupParentId_mark    : "",
	groupStatus      : "",groupStatus_mark      : "",
	groupType      : "",groupType_mark      : "",
...
function getUserConfiguration(){
	...
	for (var i=0; i<oList.length; i++){	
		...
		if (key=="group.parentid") {fcuser.groupParentId = val;fcuser.groupParentId_mark =datatype;}//2013-5-23		
		if (key=="group.status") {fcuser.groupStatus = val;fcuser.groupStatus_mark =datatype;}
		if (key=="group.type") {
			fcuser.groupType = IsSpace(val)?"'dpt'":val;//当fcuser.xml中未设置时，直接等于dpt，用于树节点图片2013-5-23
			fcuser.groupType_mark =datatype;
		}
	...
	if (bResult=="false") {
		...
		if (num==1){// 直接链接到action的view中
			...
			if (view==""){// 没有view,则出现action选择页，点击action名字，执行doAction
				...
				window.open(surl,"_parent");//2013-5-14 djframe修改成延时调用resize后，必须用parent
				// window.open(surl);
				return;				
			}
...
function show_next_step(oDom){
	...
	parent.window.open(surl,"_parent");	//2013-5-14 djframe修改成延时调用resize后，必须用parent
	return;					
}
